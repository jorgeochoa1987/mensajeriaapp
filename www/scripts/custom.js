...
function consultadom(id)
{    iduser =sessionStorage.getItem("id");
$.get("http://spmensajeria.com/admin/consultaUsuario.php?opcion=12&user="+iduser+"", function( data ) {
 console.log( data ); 
 $('#informacion').html('<a>');
 var i;
 for (i = 0; i < 1; i++) {
	...
	$('#informacion').append( '<div  class="content-box bg-white shadow-large">'+
	'<h4 class="top-25"><i class="fa fa-home font-14 top-5 color-green-dark float-right">'+ data.consumo[i].estado+'</i>Descripción: '+data.consumo[i].descripcion+'</h4>'+
	'<p class="font-12 bottom-20">Comisión:$'+ data.consumo[i].ganacia+'<br> Recibe: '+data.consumo[i].lugarS+' Destino: '+data.consumo[i].lugarS+' <br> fecha de domicilio: '+data.consumo[i].fecha+''+
   '</p>  <div class="content-box  bg-white"><div class="top-25 bottom-30"><h4>Descripción de entrega</h4><div class="input-simple-2 textarea has-icon"><i class="fa fa-edit"></i><textarea class="textarea-simple-2" placeholder="Ingrese descripción del domicilio"></textarea></div>'+
  '<div class="clear"></div></div></div><a  style="background: #4CAF50;"  onclick="entregar('+ data.consumo[i].id+')" class="button bg-highlight button-full button-rounded button-s uppercase ultrabold shadow-medium">Entregar</a></div>');    
...
{
   iden = id;
   texto = $('#textodescripion').value;
	$.get("http://spmensajeria.com/admin/domiciliosingle.php?opcion=14&id="+iden+"&des="+texto+"", function( data ) {
 console.log( data ); 

 

})
alert("Domicilio entregado");
 window.location.href = ('historial.html'); 
}
function eventoslist()
{
	 
	$.get("http://spmensajeria.com/admin/consumo.php?opcion=6", function( data ) {
		console.log( data ); 
		var i;
		for (i = 0; i < 3; i++) {
			var image ="http://spmensajeria.com/admin/aplication/modules/uploads/"+data.consumo[i].imagen;
			if(data.consumo[i].imagen=== "")
			{ 
				image ="http://spmensajeria.com/admin/aplication/modules/uploads/default.png";
			}
			console.log(image);
		  $('#eventos').append( '<div class="item shadow-large round-element">'+
						'<img width="700" class="owl-lazy" src="'+image+'" data-src="'+image+'">'+
						'<div class="above-overlay cover-content-bottom">'+
						'<h4 class="center-text uppercase ultrabold color-white bottom-0">'+data.consumo[i].nombre+'</h4>'+
						'<p class="color-white bottom-0 center-text font-12 opacity-80"></p>'+
						'</div><div class="overlay overlay-gradient"></div></div>');    
...
function counnoti()
{
	...
	$.get("http://spmensajeria.com/admin/consumo.php?opcion=13", function( data ) {
		$('#not').html('<p>');
		var i;
		for (i = 0; i < 2; i++) {
		{
			$('#not').html(data.consumo[0].total);
		}			
	  
		} 
	  }) 
}
function notificacionesdom()
{
	$.get("http://spmensajeria.com/admin/consumo.php?opcion=12", function( data ) {
	 $('#menu-bell').html('<a>');
     var i=0; 
     for (i = 0; i < 3; i++) {
         
		 
       $('#menu-bell').append( '<a onclick="solicitar('+data.consumo[i].id+')"href="#" class="bell-item" style="background: #ededed;padding: 20px;">'+
                                '<img class="preload-image shadow-icon-large" src="http://spmensajeria.com/admin/aplication/modules/uploads/default.png" data-src="http://spmensajeria.com/admin/aplication/modules/uploads/default.png">'+
                                '<strong>'+data.consumo[i].descripcion+'</strong><span class="bg-red-dark">Aceptar Domicilio</span>'+
                                '<em style="font-size:11px!important;">Lugar Salida:'+data.consumo[i].lugarS+' <br> Lugar Entrega: '+data.consumo[i].lugarL +''+
                                '<br>Valor dom</strong>: $'+data.consumo[i].Valor+'<br>Ganancia:</strong> $ '+data.consumo[i].ganacia +'</em> </a>');    
...
function solicitar(id)
{
	...
	url ="http://spmensajeria.com/admin/solicitud.php?user="+user+"&id="+id+"";
...
alert("Domicilio Asignado");
window.location.href = ('historial.html');}


function usuarioActivo()
{
	x = sessionStorage.getItem("usuario");
		console.log(x);
	if(x!== null)
	{
		//alert("Hola, "+ x);

    $('#user').html( sessionStorage.getItem("usuario"));
...
{
	x = sessionStorage.getItem("usuario");
y = sessionStorage.getItem("pass");
    $.get("http://spmensajeria.com/admin/datosusuario.php?user="+x+"&pass="+y+"", function( data ) {
        console.log( data ); 
if (data.consumo[0].estado =='0')
{
    es = "activo";
}else{
    es="inactivo";
           ...
           $('#estadoUsu').html( ''+es+'');    
           $('#estadoRec').html( ''+data.consumo[0].valor+'<br>' + 'finaliza: '+ data.consumo[0].fechafin +''); 
           $('#moto').html( ''+data.consumo[0].placa+'');     
         } )
...
function historialDom()
{
	...
	$.get("http://spmensajeria.com/admin/consultaUsuario.php?opcion=12&user="+iduser+"", function( data ) {
	 console.log( data ); 
	 $('#historialDomicilios').html('<a>');
     var i;
     for (i = 0; i < 10; i++) {
         
		  
	    $('#historialDomicilios').append( '<div  onclick="DomiciloPage('+data.consumo[i].id+')" class="content-box bg-white shadow-large">'+
	   '<h4 class="top-25"><i class="fa fa-home font-14 top-5 '+ data.consumo[i].estado+' float-right">'+ data.consumo[i].estado+'</i>Descripción: '+data.consumo[i].descripcion+'</h4>'+
	   '<p class="font-12 bottom-20">Comisión:$'+ data.consumo[i].ganacia+'<br> Recibe: '+data.consumo[i].lugarS+' Destino: '+data.consumo[i].lugarS+' <br> fecha de domicilio: '+data.consumo[i].fecha+''+
	  '</p></div>');     
